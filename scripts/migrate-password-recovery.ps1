...
$TableId = 'password_recovery_challenges'
...
function Ensure-Table {
  ...
  & appwrite tables-db create-table --database-id $DatabaseId --table-id $TableId --name 'Password recovery challenges' --row-security true | Out-Null
...
Wait-Columns @('userId', 'emailHash', 'ipHash', 'codeHash', 'linkHash', 'expiresAt', 'attempts', 'emailMessageId', 'createdAt')
Ensure-Index recovery_user @('userId')
Ensure-Index recovery_email @('emailHash')
Ensure-Index recovery_ip @('ipHash')

Write-Host 'One-hour password recovery schema migration submitted.'
