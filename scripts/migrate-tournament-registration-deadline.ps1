...
if ($columns.key -contains 'registrationDeadline') {
  Write-Host 'tournaments.registrationDeadline already exists.'
  exit 0
}
...
if ($LASTEXITCODE -ne 0) {
  throw 'Could not create tournaments.registrationDeadline.'
}

$deadline = (Get-Date).AddSeconds(90)
do {
  Start-Sleep -Seconds 2
  $column = Get-TournamentColumns | Where-Object key -eq 'registrationDeadline'
} while ($column.status -ne 'available' -and (Get-Date) -lt $deadline)

if ($column.status -ne 'available') {
  throw 'tournaments.registrationDeadline did not become available before the timeout.'
}

Write-Host 'Tournament registration deadline migration complete.'
